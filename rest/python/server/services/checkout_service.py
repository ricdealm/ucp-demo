...
from ucp_sdk.models.schemas.ucp import ResponseCheckoutSchema, ResponseOrderSchema, Version
from ucp_sdk.models.schemas.capability import ResponseSchema as CapabilityResponseSchema
from ucp_sdk.models.schemas.shopping.discount import Allocation, AppliedDiscount, DiscountsObject
...
from ucp_sdk.models.schemas.shopping.types.fulfillment_destination import FulfillmentDestination
from ucp_sdk.models.schemas.shopping.types.shipping_destination import ShippingDestination
...
from ucp_sdk.models.schemas.shopping.types.token_credential import TokenCredential as TokenCredentialResponse
...
class CheckoutService:
  ...
  async def create_checkout(
    ...
    if checkout_req.fulfillment:
      ...
      fulfillment_resp = Fulfillment(methods=resp_methods)

    checkout = Checkout(
      ucp=ResponseCheckoutSchema(
        version=Version(config.get_server_version()),
        capabilities={
          ReverseDomainName(root="dev.ucp.shopping.checkout"): [
          ...
          if method_type == "shipping":
            if m_req.destinations:
              ...
              for dest_req in m_req.destinations:
                ...
                resp_destinations.append(
                  FulfillmentDestination(
                    root=ShippingDestination(**dest_data)
                  )
                )
  ...
  async def complete_checkout(
    ...
    try:
      ...
      order = Order(
        ucp=ResponseOrderSchema(**checkout.ucp.model_dump()),
  ...
  async def _get_and_validate_checkout(self, checkout_id: str) -> Checkout:
    ...
    # Hack to fix Pydantic RootModel serialization as dict keys
    if "ucp" in data and "capabilities" in data["ucp"]:
      ...
      for k, v in caps.items():
        if k.startswith("root='") and k.endswith("'"):
          ...
          new_caps[k] = v
      data["ucp"]["capabilities"] = new_caps
