...
async def create_checkout(
  ...
  if "ucp" in result_dict and "capabilities" in result_dict["ucp"]:
    caps = result_dict["ucp"]["capabilities"]
    new_caps = {}
    for k, v in caps.items():
      if k.startswith("root='") and k.endswith("'"):
        ...
        new_caps[k] = v
    result_dict["ucp"]["capabilities"] = new_caps
...
async def complete_checkout(
  ...
  ap2: Annotated[dict | None, Body()] = None,
) -> dict[str, Any]:
